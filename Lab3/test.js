      ...
      geocoder.geocode({'location': loc}, function(results, status) {
          if (status === 'OK') {
            if (results[1]) {
              ...
              panel.style.display = 'block';
              curr.innerHTML = results[1].formatted_address;
	      console.log("RT " + rt);//collect(setLat, setLong);
	       if (rt === false) { update1(); } else { meh = true; update(); }
            } else {
              window.alert('No results found');
            }
          } else {
	...
	console.log("pos is" + pos);
     ...
     //}
      ...
      geocoder.geocode({'location': pos}, function(results, status) {
		...
		window.alert('this is what is happening VUNNNN');
            window.alert('Geocoder failed due to: ' + status);
          }
        });
      geocoder.geocode({'location': pos2}, function(results, status) {
		...
		window.alert('this is what is happening TOOOO');
            window.alert('Geocoder failed due to: ' + status);
          }
        });
    }, function() {
